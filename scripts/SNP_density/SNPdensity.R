...
frags <- read.csv(SNPfile, header=TRUE, sep='\t', stringsAsFactors = FALSE)
...
temp <- m[,2]
temp$SNPspace <- width(m[,1])
q <- findOverlaps(binGR, temp, type='equal')
indices <- queryHits(q)
binGR$SNPspace <- rep(0, times=length(binGR))
#takes about a minute
j <- sapply(unique(queryHits(q)), function(i) binGR[i]$SNPspace <<- sum(temp[queryHits(q)==i]$SNPspace))
final <- data.frame(Chr=seqnames(binGR), Start=start(binGR), End=end(binGR), SNPDensity=round(binGR$SNPspace/(2*width(binGR)), 4))#since the basepai calculation calculte twice for the SNPs

write.table(final, file=sprintf("SNPdensity.%s.%dkb.tsv", genome, windowSize/1000), row.names = FALSE, col.names=FALSE, quote=FALSE, sep="\t")
write.table(final, file=sprintf("SNPdensity.%s.%dkb.csv", genome, windowSize/1000), row.names = FALSE, col.names=TRUE, quote=FALSE, sep=",")
